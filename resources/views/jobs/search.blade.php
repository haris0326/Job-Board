            ...
            @if ($searches->count() > 0)
                @foreach ($searches as $job)
                    <li class="job-listing d-block d-sm-flex pb-3 pb-sm-0 align-items-center">
                        <a href="{{ route('single.job', $job->id) }}" class="d-block w-100"></a>
                        <div class="job-listing-logo">
                            <!-- Provide a fallback image if $job->image is empty or not set -->
                            <img src="{{ asset('assets/images/' . $job->image) }}" alt="{{ $job->job_title }} - {{ $job->company }}" class="img-fluid">
                        </div>
